...
if(isset($_POST['add'])){
	...
	if($grading == "First Grading"){
		...
		$conn = mysqli_connect("localhost", "username", "password", "grading_system");
		mysqli_query($conn, $query);
	} elseif ($grading == "Second Grading"){
		$query = "UPDATE records SET second_grading='".$grade."' where id='".$id."'";
		$conn = mysqli_connect("localhost", "username", "password", "grading_system");
		mysqli_query($conn, $query);
	} elseif ($grading == "Third Grading"){
			...
			<tr>
				<th>
					<table width="100%" cellspacing="0">
						<tr>
							<th colspan="4" style="border-bottom: 1px solid;background-color: #28FCE1;padding: 5px 0px;font-size: 25px;">Add Students Grade for <?php echo $grading;?></th>
						</tr>
						<tr>
